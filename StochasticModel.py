...
simulation_time = 1500  # Define the simulation time             #changed
dt = 0.1                                                         #changed
...
for k in range(Number_of_Protein):                               #changed
    if (k == 0) : 
        activities[0, k] = 1
    else:
        activities [0, k] = 0.1
...
sigma = np.full(5, 0.5)                               #moderate stochasticity (changed)
...
bounds = [(0, 1),(0, 1),(0, 1)]
pkg_initial = [0.48254883, 0.46549677, 0.55996092]
# Constraint function
#def constraint(p):
...
plt.title("Stochastic Evolution")
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')  
plt.savefig("Stochastic Evolution.png", dpi=300)
plt.close()
...
plt.legend()
plt.title("Stochastic Inhibition and Proliferation")
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
plt.savefig("Stochastic Inhibition and Proliferation.png", dpi=300)
plt.close()